...
import { STEAM_USER_ID } from "./constants";
...
export async function fetchSteamData(): Promise<SteamResult> {
  ...
  const steamId = import.meta.env.STEAM_USER_ID || STEAM_USER_ID;
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[steam] HTTP ${res.status} ${res.statusText} from Steam API`);
      return { games: [], totalPlaytime: 0 };
    }
    ...
    const resolved = await Promise.all(
      raw.map(async (g) => {
        ...
        return {
          ...
          playtimeTwoWeeks: g.playtime_2weeks
