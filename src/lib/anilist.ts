...
export interface RecentActivityEntry {
  ...
  progress: string;
  label: string;
  /** Unix timestamp in seconds (as returned by the AniList API). */
  createdAtSec: number;
}
...
async function anilistGraphql<T>(query: string, variables: Record<string, unknown>): Promise<T | null> {
  try {
    ...
    if (!response.ok) {
      console.warn(`[anilist] HTTP ${response.status} from AniList API`);
      return null;
    }
...
export async function fetchAniListSnapshot(): Promise<AniListSnapshot> {
  ...
  if (activityData?.Page?.activities) {
    for (const entry of activityData.Page.activities) {
      ...
      recentActivity.push({
        ...
        createdAtSec: entry.createdAt ?? 0,
      });
    }
  } else {
      ...
      for (const list of fallbackData.MediaListCollection.lists) {
        for (const entry of list.entries || []) {
          ...
          recentActivity.push({
            ...
            createdAtSec: entry.updatedAt ?? 0,
          });
        }
      }
    }
  ...
  recentActivity.sort((a, b) => b.createdAtSec - a.createdAtSec);

  const snapshot: AniListSnapshot = {
    currentlyWatching: currentlyWatchingTitles,
    recentActivity: recentActivity.slice(0, 5),
  };
  _anilistCache = { data: snapshot, ts: Date.now() };
  return snapshot;
}
