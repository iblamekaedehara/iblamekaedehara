 ...
 * - INIT_STATE is a FULL REPLACE — it's always a complete snapshot from Lanyard.
...
export function getPresenceTransport(): IPresenceTransport {
...
class PresenceTransport implements IPresenceTransport {
  ...
  connect(): void {
    ...
    this.ws.onopen = () => {
      this.helloTimeoutTimer = setTimeout(() => {
        console.warn("[presence-transport] Hello timeout — closing stale socket");
        this.ws?.close();
      }, 10_000);
    };
  ...
  getState(): PresenceState {
    return {
      ...this.currentState,
      activities: [...(this.currentState.activities ?? [])],
    };
  }
  ...
  private handleMessage = (event: MessageEvent): void => {
    try {
      ...
      if (msg.op === 1) {
        ...
        if (import.meta.env.DEV) console.log("[presence-transport] Hello received, heartbeat_interval:", msg.d?.heartbeat_interval);
      ...
      if (msg.op === 0 && (msg.t === "INIT_STATE" || msg.t === "PRESENCE_UPDATE")) {
        if (import.meta.env.DEV) console.log("[presence-transport] Raw payload msg.t:", msg.t, "msg.d:", JSON.stringify(msg.d));
        const validated = validatePresencePayload(msg.d);
        if (validated) {
          if (import.meta.env.DEV) console.log("[presence-transport] Validated. activities count:", validated.activities?.length, "activities:", validated.activities);
          this.sequenceId++;

          if (msg.t === "INIT_STATE") {
            // Full replace — INIT_STATE is always a complete snapshot
            this.currentState = {
              status: validated.discord_status ?? "offline",
              user: validated.discord_user ?? null,
